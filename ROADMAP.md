...
| 0 — Recovery test | **PASSED 2026-09-06.** Power-cycled with the stick in; stock CoreELEC booted from USB and the internal install was untouched. Caveat found by doing it: a box booted from the stick could not be reached over SSH at all. Not a Wi-Fi problem - the box is wired - most likely SSH being off in a fresh `/storage`. Assume no remote help during a recovery |
...
| 4 — Hardware rendering proof | **Path reached; the core crashes in it.** With an N64 ROM and `mupen64plus-rdp-plugin=gliden64`, `CRetroPlayerRendering::OpenStream()` runs — the log shows `Configuring format unknown, nominal 640x480`, which is our code. Then SIGSEGV, and the whole backtrace is inside `mupen64plus-nx.so`: `retro_run()` → GLideN64 → `strstr()`. That is `strstr(glGetString(GL_EXTENSIONS), …)` on a NULL, i.e. **no current GL context on the game loop thread**. Our pool creates its EGL context lazily in `CreateRenderBuffer()`, which only runs when the core first asks for a framebuffer — *after* it has already queried extensions. The context has to be made current before the core's `context_reset`, not on first buffer request. Exactly the ordering Kodi's own `OpenGL_Roadmap.md` warns about under "GL Context Management" |
