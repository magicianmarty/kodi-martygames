...
| 4 — Hardware rendering | **Restructured, builds clean, not yet run.** Five ordering defects were fixed by inference before reading how RetroArch does it (`HWRENDER.md`): it allocates **one** FBO up front at driver init and `get_current_framebuffer` is a lookup that cannot fail, while its `SET_HW_RENDER` handler creates nothing. Ours allocated from a pool on first request, racing the client's `context_reset` — every defect was a consequence. Patch 1029 rebuilds it the RetroArch way. Known remaining gap: the Kodi game API has no hardware equivalent of `game_stream_video_properties`, so nothing carries the framebuffer's dimensions — `OpenStream()` hardcodes 640x480 and the wrapper asks with `GetBuffer(0, 0)`. Fine for N64 native, wrong for any core that upscales |
