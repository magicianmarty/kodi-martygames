...
| 4 — Hardware rendering | **Negotiated and stable; frames not yet presented.** N64 now loads and runs without crashing: `Enabling hardware rendering for OpenGLES 3.0`, the shared EGL context is created, and the R4300 dynarec emulates at **93% GameLoop CPU**. Screen is still black. No renderer is ever created for the FBO pool, so `CRPRenderManager::GetCurrentFramebuffer()` — which only considers pools where `HasVisibleRenderer()` — never hands the client a buffer. That is a chicken-and-egg the software path dodges by caching frames in `AddFrame()` until the GUI asks to render. `Create()` needs to force a buffer and renderer into existence up front. **That is the last piece** |
